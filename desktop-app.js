...
function currentTourLayer(){
  const btn = qs(".layer.active");
  if(btn) return btn.dataset.layer;
  if(IS_FEIYI_PAGE) return "feiyi_national";
...
let activeLayers = new Set(IS_TOUR_PAGE ? ["tour_scenic"] : IS_FEIYI_PAGE ? ["feiyi_national"] : ["tree"]);
...
function pointTitle(p){
  if(IS_TOUR_PAGE || IS_FEIYI_PAGE) return p.name || p.id;
...
function selectPoint(id){
  ...
  if(IS_FEIYI_PAGE){
    ...
    if(!activeLayers.has(layer)){
      ...
      const btn = qs(`.layer[data-layer="${layer}"]`);
      if(btn) btn.classList.add("active");
    }
    
    qsa(".feiyi-detail").forEach(el => el.remove());
    
    const itemEl = qs(`.item[data-id="${esc(id)}"]`);
    if(itemEl){
...
function initAmap(){
  ...
  script.src = `https://webapi.amap.com/maps?v=2.0&key=${encodeURIComponent(window.AMap_KEY)}&plugin=AMap.Scale,AMap.ToolBar,AMap.Geocoder&callback=onAmapReady`;
...
window.onAmapReady = function(){
  if(IS_FEIYI_PAGE){
    statusEl.innerHTML = "高德地图已加载，点击非遗项目点位可查看详情。";
  ...
  if(IS_FEIYI_PAGE){
    ...
    const visibleData = currentTourData();
    const visibleIds = new Set(visibleData.map(p => p.id));
    markers = dataSource.filter(p => Number.isFinite(p.lng) && Number.isFinite(p.lat)).map(p => {
      ...
      const marker = new AMap.Marker({
        ...
        map: (activeLayers.has(currentLayer) && visibleIds.has(p.id)) ? map : null
      });
    ...
    // 文旅景点：使用高德 REST API POI 搜索定位村子（村委会）
    // 直接 fetch restapi.amap.com，绕过 JS API 安全密钥限制
    // 通过搜索村委员会的位置，把标点放在村子名字旁边
    if(IS_TOUR_PAGE){
      ...
      if(allPoints.length){
        ...
        const processNext = () => {
          if(queue.length === 0){
            ...
            console.log(`[景点 POI] 全部完成: 成功 ${geocodedCount}/${total}, 失败 ${failedCount}`);
            const currentLayer = currentTourLayer();
          ...
          const p = queue.shift();
          // 提取搜索关键字：优先用 village，其次从 address 提取村子名
          const keyword = buildVillageKeyword(p);
          // POI 搜索：搜索村委会（typecode=130106），限制在长治市
          const apiUrl = `https://restapi.amap.com/v3/place/text?key=${encodeURIComponent(window.AMap_KEY)}&keywords=${encodeURIComponent(keyword)}&city=${encodeURIComponent("长治市")}&citylimit=true&types=130106&output=JSON`;
          ...
          fetch(apiUrl)
            ...
            .then(data => {
              ...
              if(data && data.status === "1" && data.info === "OK" && data.pois && data.pois.length){
                // 只选择上党区的村委会，避免定位到其他区的同名村子
                const sdPoi = data.pois.find(poi => poi.adname === "上党区");
                if(sdPoi){
                  const locStr = sdPoi.location; // "lng,lat"
                  ...
                  if(Number.isFinite(newLng) && Number.isFinite(newLat)){
                    ...
                    failedCount++;
                    console.warn(`[景点 POI] ${p.id} 坐标解析失败: ${locStr}`);
                  }
                } else {
                  // 上党区没有找到村委会，用 Geocoder 回退搜索完整地址
                  const fullAddress = buildHistoricFullAddress(p);
                  const geoUrl = `https://restapi.amap.com/v3/geocode/geo?key=${encodeURIComponent(window.AMap_KEY)}&address=${encodeURIComponent(fullAddress)}&city=${encodeURIComponent("长治市")}`;
                  fetch(geoUrl)
                    .then(r2 => r2.json())
                    .then(data2 => {
                      if(data2 && data2.status === "1" && data2.info === "OK" && data2.geocodes && data2.geocodes.length){
                        const locStr = data2.geocodes[0].location;
                        const [newLng, newLat] = locStr.split(",").map(Number);
                        if(Number.isFinite(newLng) && Number.isFinite(newLat)){
                          geocodedCount++;
                          const item = markerById.get(p.id);
                ...
                failedCount++;
                console.warn(`[景点 POI] ${p.id} 失败: infocode=${data && data.infocode}, info=${data && data.info}, keyword=${keyword}`);
              }
              statusEl.innerHTML = `正在通过地址精确定位景点（${completedCount}/${total}）...`;
              setTimeout(processNext, 400); // 串行：每个请求间隔 400ms，规避 QPS 限制
            })
            .catch(err => {
              completedCount++;
              failedCount++;
              console.warn(`[景点 POI] ${p.id} 网络错误:`, err.message);
              statusEl.innerHTML = `正在通过地址精确定位景点（${completedCount}/${total}）...`;
...
function toggleLayer(layerName){
  ...
  if(IS_FEIYI_PAGE){
    ...
    qsa(".layer").forEach(b => b.classList.toggle("active", b.dataset.layer === layerName));
    activeId = null;
    qs("#fallbackPopup")?.classList.remove("visible");
